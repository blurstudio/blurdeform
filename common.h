...
/**
  Get the barycentric coordinates of point P in the triangle specified by points
  A,B,C.
...
/**
  Get the vertex adjacency of the specified mesh.  The vertex adjacency are the
  vertex ids of the connected faces of each vertex.
...
/**
  Calculates a weight for each vertex within the crawl sample radius.  Vertices
  that are further away from the origin should have a lesser effect than
  vertices closer to the origin.
...
/**
  ...
  @param[in] triangleVertices The vertex ids forming the triangle of the closest
  point.
...
/**
  ...
  @return A __m256d vector where each element is the corresponding p vector dot
  product with w.
 */
template <typename T>
__m256d Dot4(
    double w1, double w2, double w3, double w4, const T &p1, const T &p2,
...
)
{
    ...
    // low to high: xw0+xw1+xw2+xw3 yw0+yw1+yw2+yw3 zw0+zw1+zw2+zw3
    // ww0+ww1+ww2+ww3
